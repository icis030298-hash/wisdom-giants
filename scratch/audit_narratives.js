...
const missingSlugs = masterSlugs.filter(slug => !narratives[slug]);
...
// Check for extra/mismatched keys in narratives
const extraKeys = narrativeKeys.filter(key => !masterSlugs.includes(key));
console.log(`\nExtra/Mismatched Keys in final-narratives.json (${extraKeys.length}):`);
extraKeys.forEach(key => {
  console.log(`- ${key}`);
});

// Check for parity
if (missingSlugs.length === 0 && extraKeys.length === 0) {
