...
// Create chunks of 10
const chunks = [];
for (let i = 0; i < missingGiants.length; i += 10) {
  chunks.push(missingGiants.slice(i, i + 10));
}

const prompts = chunks.map((chunk, i) => {
  let prompt = `Your task is to generate illustrations for the following giants using the \`generate_image\` tool and the Strict Flat Vector prompt.\n\n`;
...
3. After generating the image, use the \`run_command\` tool to copy the artifact to \`c:\\Users\\natey\\Desktop\\wisdom-giants\\public\\images\\giants\\[slug].png\`. Use \`cmd /c copy "source" "destination"\`.
