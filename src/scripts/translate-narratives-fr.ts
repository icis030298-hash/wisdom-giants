...
function translateBatch(batch: Record<string, GiantNarrative>): Promise<any> {
  return new Promise((resolve, reject) => {
    const prompt = `You are an expert translator and historian. Translate the following JSON object containing historical biographies and narratives from English to elegant, natural, and historically accurate French.
For each giant in the object, translate the English fields into their corresponding French fields. Keep the exact same structure but change the _en suffix to _fr:
- era_en -> era_fr
- epic_en -> epic_fr
- trials_en -> trials_fr
- overcoming_en -> overcoming_fr
- inside the wisdom array:
  - quote_en -> quote_fr
  - meaning_en -> meaning_fr

IMPORTANT POLICING RULES:
1. Always use polite, formal French vouvoiement (use "vous", "votre", "vos" instead of "tu", "ton", "tes") for any direct address in greetings, meaning or quotes.
2. Keep the narrative tone monumental, inspiring, and appropriate for historical figures.
3. Keep core quotes historically recognized in French if applicable (e.g., Napoleon: "Impossible n'est pas français", Julius Caesar: "Le sort en est jeté").

Do NOT include the original _en or _ko or _es fields in your output. Return ONLY the newly translated _fr fields for each giant, matching this structure exactly:
{
  "steve-jobs": {
    "era_fr": "...",
    "epic_fr": "...",
...
${JSON.stringify(batch, null, 2)}`;
    ...
    const options = {
      ...
      method: 'POST',
      timeout: 120000, // 120 seconds socket timeout (2 minutes)
      headers: {
...
async function run() {
  console.log("Starting French Narrative Translation...");
  ...
  const giantsToTranslate = slugs.filter(slug => {
    ...
    return !data.epic_fr; // Needs translation if epic_fr is missing
  });

  console.log(`Of these, ${giantsToTranslate.length} giants need French narrative translation.`);

  if (giantsToTranslate.length === 0) {
    console.log("🎉 All giants already have French narratives!");
    process.exit(0);
  }

  // Biography text is large, so a batch size of 9 is safe and prevents token limit or timeout issues, while optimizing API usage to bypass the 20 RPD limit.
  const batchSize = 9; 
    ...
    try {
      ...
      for (const slug of batchSlugs) {
        if (translatedBatch[slug]) {
          ...
          narrativesData[slug].era_fr = tData.era_fr;
          narrativesData[slug].epic_fr = tData.epic_fr;
          narrativesData[slug].trials_fr = tData.trials_fr;
          narrativesData[slug].overcoming_fr = tData.overcoming_fr;
          
          if (tData.wisdom && narrativesData[slug].wisdom) {
            for (let j = 0; j < narrativesData[slug].wisdom.length; j++) {
              if (tData.wisdom[j]) {
                narrativesData[slug].wisdom[j].quote_fr = tData.wisdom[j].quote_fr;
                narrativesData[slug].wisdom[j].meaning_fr = tData.wisdom[j].meaning_fr;
              }
            }
          }
