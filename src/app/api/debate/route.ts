...
${baseGuidelines}
必ず極めて自然で格調高い「日本語」で返答してください。歴史的な口調や尊厳を完全に再現してください。
性格と哲学：
${persona}`;
    } else if (locale === 'es') {
...
${persona}`;
    ...
    // Convert debate history into format suitable for Gemini chat
    // Since Gemini models expect alternating user/model roles, we can represent the debate as a chat session.
    // To feed the entire history, we will compile it into parts of a single session, or send the entire history as text contents.
    // The most robust way to prompt a multi-turn multi-agent debate in Gemini is to build the previous debate log as part of the prompt message.
...
`;
    ...
    // Try Gemini Models in sequence (cost-efficient model gemini-2.5-flash-lite first, then fallback)
