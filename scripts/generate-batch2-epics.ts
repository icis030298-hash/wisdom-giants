...
const batch2Giants = ["kimpa-vita","seneca","augustus","imhotep","rabban-bar-sauma","selim-iii","thomas-edison","xuanzang","louis-pasteur","alan-turing","emmeline-pankhurst","james-clerk-maxwell","cetshwayo-kampande","homer","sergei-korolev","friedrich-schiller","john-f-kennedy","olympe-de-gouges","simone-weil","mustafa-kemal-ataturk","franklin-d-roosevelt","pachacuti","ahmad-yasawi","john-locke","johannes-gutenberg"];
...
- **[시점 분리 가드레일]**: 인물의 사망 시점과 그 이후에 발생한 사건(후계자 시대의 사건, 사후 평가, 유산 형성 과정)을 명확히 구분해서 서술할 것. '그의 죽음 이후', '그의 후계자는', '사후' 같은 시점 표지어를 사용해 인물 생전 사건과 사후 사건이 뒤섞이지 않도록 할 것. 특히 인물이 죽거나 실각한 후 벌어진 전투/사건을 본인이 직접 겪은 것처럼 서술하지 않도록 주의.
...
async function main() {
  console.log("=== Vertex AI 기반 서사 확장 (배치 2 part2 - 25명, SEO/GEO 최적화) ===");
  ...
  for (let i = 0; i < batch2Giants.length; i++) {
    const slug = batch2Giants[i];
    console.log(`\n[${i + 1}/${batch2Giants.length}] Processing slug: ${slug}...`);
  ...
  const draftOutputPath = path.resolve('scratch/batch2-part2-narratives-draft.json');
