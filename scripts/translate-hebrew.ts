...
async function main() {
  console.log("=== Starting Hebrew Translation Worker ===");
  ...
  for (const slug of slugs) {
    ...
    localTotal++;
    console.log(`[Hebrew] Translating ${slug} (${localTotal}/${slugs.length})...`);
    ...
    if (res.success) {
      ...
      console.error(`[Hebrew] Failed to translate ${slug}: ${res.reason}`);
    }
  ...
  console.log("=== Hebrew Translation Finished and Saved! ===");
}

main().catch(err => {
  console.error("Fatal error in Hebrew translation:", err);
  process.exit(1);
});
