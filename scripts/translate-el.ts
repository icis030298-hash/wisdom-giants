...
async function main() {
  console.log("=== Starting Greek Translation Worker ===");
  ...
  for (const slug of slugs) {
    ...
    localTotal++;
    console.log(`[Greek] Translating ${slug} (${localTotal}/${slugs.length})...`);
    ...
    if (res.success) {
      ...
      console.error(`[Greek] Failed to translate ${slug}: ${res.reason}`);
    }
  ...
  console.log("=== Greek Translation Finished and Saved! ===");
}

main().catch(err => {
  console.error("Fatal error in Greek translation:", err);
  process.exit(1);
});
