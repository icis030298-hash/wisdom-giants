...
async function main() {
  console.log("=== Starting Turkish Translation Worker ===");
  ...
  for (const slug of slugs) {
    ...
    localTotal++;
    console.log(`[Turkish] Translating ${slug} (${localTotal}/${slugs.length})...`);
    ...
    if (res.success) {
      ...
      console.error(`[Turkish] Failed to translate ${slug}: ${res.reason}`);
    }
  ...
  console.log("=== Turkish Translation Finished and Saved! ===");
}

main().catch(err => {
  console.error("Fatal error in Turkish translation:", err);
  process.exit(1);
});
