...
async function main() {
  console.log("=== Starting Hausa Translation Worker ===");
  ...
  for (const slug of slugs) {
    ...
    localTotal++;
    console.log(`[Hausa] Translating ${slug} (${localTotal}/${slugs.length})...`);
    ...
    if (res.success) {
      ...
      console.error(`[Hausa] Failed to translate ${slug}: ${res.reason}`);
    }
  ...
  console.log("=== Hausa Translation Finished and Saved! ===");
}

main().catch(err => {
  console.error("Fatal error in Hausa translation:", err);
  process.exit(1);
});
