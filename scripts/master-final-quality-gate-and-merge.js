...
const lastDir = path.join(__dirname, '..', 'scratch', 'translations_last');
const lastFiles = [
  'pl_31.json',
  'others_59.json'
];
...
lastFiles.forEach(fileName => {
  const filePath = path.join(lastDir, fileName);
...
console.log(`Loaded last batch candidate items: ${lastItems.length} (Expected 90)`);
...
console.log(`\n======================================================`);
console.log(`=== FINAL ROUND AUTOMATED PRE-MERGE QUALITY GATE RESULTS ===`);
console.log(`======================================================`);
console.log(`TOTAL CANDIDATE ITEMS EVALUATED : ${lastItems.length}`);
...
if (rejectedItems.length > 0) {
  ...
  console.log('\n[PERFECT SUCCESS]: 0 items rejected! 100% of 90 translated posts passed all Quality Gate rules (a)-(f)!');
}
...
['translations', 'translations_rem', 'translations_final', 'translations_last', 'tasks', 'tasks_rem', 'tasks_final', 'tasks_last'].forEach(subDir => {
