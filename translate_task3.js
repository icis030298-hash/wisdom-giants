  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (item.timeline) {
      for (const t of item.timeline) {
        outItem.timeline.push({
          year: t.year,
          event: await doTranslate(t.event, 'id')
        });
        await sleep(200);
      }
    }
    ...
    if (item.keyAchievements) {
      for (const a of item.keyAchievements) {
        outItem.keyAchievements.push({
          title: await doTranslate(a.title, 'id'),
          description: await doTranslate(a.description, 'id')
        });
        await sleep(200);
      }
    }
    ...
    if (item.faq) {
      for (const f of item.faq) {
        outItem.faq.push({
          question: await doTranslate(f.question, 'id'),
          answer: await doTranslate(f.answer, 'id')
        });
        await sleep(200);
      }
    }
