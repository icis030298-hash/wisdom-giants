...
def translate_text(text):
    ...
    if text.strip() == "":
        return text
    try:
        translated = ts.translate_text(text, from_language='ko', to_language='ha', translator='bing')
        return translated
    except Exception as e:
        print(f"Error translating {text}: {e}")
        time.sleep(1)
        try:
            return ts.translate_text(text, from_language='ko', to_language='ha', translator='google')
        except:
            return text
